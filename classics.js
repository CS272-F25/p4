...
// Define your classicMovies grouped by era
const classicMovies = {
  all: [
...
function createMovieCard(movie) {
  ...
  card.innerHTML = `
    ...
    <div class="movie-info">
      ...
      <button class="btn-add-to-cart" data-movie-id="${movie.id}" aria-label="Add ${movie.title} to cart">
        Add to Cart
      </button>
    </div>
  `;
...
function displayClassics(era = 'all') {
  ...
  const movies = classicMovies[era] || classicMovies['all'];

  movies.forEach(movie => {
    const card = createMovieCard(movie);
    classicsGrid.appendChild(card);
  });
}
